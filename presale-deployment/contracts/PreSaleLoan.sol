...
contract PresaleLoan is Initializable,  OwnableUpgradeable, ReentrancyGuardUpgradeable,PausableUpgradeable {
   ...
   */
  function buyWithEdu(uint256 amount, bool stake) external payable checkSaleState(amount) whenNotPaused nonReentrant returns (bool) {
  ...
  function manageTimeDiff() internal {
      rounds[0][currentStep] = block.timestamp + currentStep * timeConstant;
  }

...
contract V2_LoanOffer is ERC721Holder, Ownable {
    ...
    function acceptBorrowRequest(uint256 requestId) external payable {
        ...
        try nft.safeTransferFrom(request.lender, address(this), request.tokenId) {
            ...
            emit TransferFailed(request.lender, address(this), request.tokenId);
    ...
    function returnNFT(address nftContract, uint256 tokenId) external payable {
        ...
        try IERC721(nftContract).safeTransferFrom(address(this), lending.lender, tokenId) {
            ...
            emit TransferFailed(address(this), lending.lender, tokenId);
            revert("NFT transfer failed");
        }
    }
